...
def _code_version() -> str:
    """Git commit of the running checkout, or '' outside a checkout."""
    try:
...
def _cache_key(text: str, max_suggestions: int | None, fmt: str) -> str:
    ...
    # The code version too: a deploy that changes trigger/suggestion logic must
    # not keep serving results the old code produced (the disk cache otherwise
    # outlives any restart).
    model = _provider.model_name if _provider else ""
    raw = f"{_CODE_VERSION}|{model}|{fmt}|{max_suggestions}|{text}"
